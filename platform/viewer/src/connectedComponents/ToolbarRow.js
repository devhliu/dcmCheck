...
class ToolbarRow extends Component {
  ...
  static propTypes = {
    ...
    handleSidePanelToggle: PropTypes.func.isRequired,
  ...
  onPressToggle() {
    this.props.handleSidePanelToggle();
  }
      ...
      <div
        className={
          this.props.className
            ? `${this.props.className} ToolbarRow`
            : 'ToolbarRow'
        }
      >
        <div className="pull-left m-t-1 p-y-1" style={{ padding: '10px' }}>
          <RoundedButtonGroup
            ...
            onValueChanged={() => this.onPressToggle()}
          />
        </div>
        {buttonComponents}
...
function _handleBuiltIn(button) {
  ...
  if (options.behavior === 'DOWNLOAD_SCREEN_SHOT') {
    commandsManager.runCommand('showDownloadViewportModal', {
      title: t('Download / Upload Key Image'),
    });
  }
}
