  ...
  return (
    <MiniDrawer>
      <div className='UploadFile_Div'>
        <Card className={`UploadFile_Card ${CloudCardclasses.root}`} style={{ paddingLeft: '6%', paddingRight: '6%', paddingTop: 20, border: '1px solid #d1cfcf', boxShadow: 'rgba(0, 0, 0, 0.24) 0px 3px 8px' }}>
          <h2 style={{ fontFamily: 'Roboto' }}>Select Bucket</h2>

          <div className="upload-file-conatiner" style={{ marginTop: 20 }} >
            <select id="bcketname" className="form-control" style={{ padding: 5, paddingRight: 10, height: 40 }}>
        ...
        system
            ...
            <div className="upload-btn-wrapper" style={{ border: '1px dashed #E2E2E2', marginTop: '40px', borderRadius: 5, paddingTop: 17, paddingBottom: 25, cursor: 'pointer', height: 140 }}>
              <BackupIcon style={{ fontSize: '50px', color: '#0a2142' }} />
              <h3 style={{ color: 'gray', marginTop: 10 }}>Select a Folder or browse</h3>
          ...
          <div style={{ textAlign: 'end', marginTop: '20px' }}>
            {Files &&
              <Button onClick={handleCancel} variant="outlined" className="UploadFilePageCancelButton">Cancel</Button>
            }
            {/* {Files &&
            <Button onClick={handleCancel} variant="outlined" style={{ padding: '8px 40px 7px 40px', marginRight: '10px' }}>Cancel</Button>
            } */}
            <Button className="UploadFilePageUploadButton" disabled={Files ? false : true} onClick={() => BrowseUpload(Files)} variant="contained" color="primary" >
              Upload
            </Button>
...
function BrowseUpload(files) {
  if (files.length > 0) {
    if (window.FormData !== undefined) {
      ...
      request.open(
        'POST',
        'https://app.dcmcloud.com/api/files/uploadserver'
      );
