...
export default function MiniDrawer(props) {
  ...
  const StripeMonthlyDetails = async () => {
    ...
    const hello = await fetch(
      `https://dcm-cloud.herokuapp.com/api/v1/stripe/customer`,
      requestOptions
    );
    ...
    if (resp.invoice) {
      ...
      setendTime(
        `Valid Till: ${date2.getDate()}  ${monthName}  ${date2.getFullYear()}`
      );
    } else {
  ...
  var subscribePack = (
    <Link style={{ fontSize: 14, color: 'whitesmoke' }} to="/package">
      SUBSCRIBE PACKAGE
    </Link>
  );
  ...
  const userDetail = async () => {
    ...
    // setLoader(true);
    const hello = await fetch(
      `https://dcm-cloud.herokuapp.com/api/v1/users/getSpecific`,
      requestOptions
    );
    const resp = await hello.json();
  ...
  const getStudyListData = async () => {
    if (JSON.parse(localStorage.getItem('StudyListData')) != null) {
      ...
      console.log(
        'StudyListData>',
        JSON.parse(localStorage.getItem('StudyListData'))
      );
    }
  };
      ...
      {token ? (
        <div className={classes.root}>
          ...
          >
            ...
            ) : // this is for PayAsYouGo studyData is greater than StripeCount
            ListData.length > StripeCount && StripeId != 'free' ? (
              ...
              >
                <span>PayAsYouGo 0 uploads remaining</span>&nbsp;
                <strong>{subscribePack}</strong>
              </div>
            ) : // this is for PayAsYouGo studyData is less than StripeCount
            ListData.length <= StripeCount && StripeId != 'free' ? (
              <div
                style={{
                  height: 24,
              ...
              >
                <span>
                  PayAsYouGo {StripeCount - ListData.length} uploads remaining
                </span>
                &nbsp;<strong>{subscribePack}</strong>
              </div>
            ) : (
              // this is for Free package studyData is less than StripeCount
              <div
                style={{
                  height: 24,
                  display: 'flex',
              ...
              >
                <span>
                  Free Package {StripeCount - ListData.length} uploads remaining
                </span>
                &nbsp;<strong>{subscribePack}</strong>
              </div>
            )}
            ...
            <Grid item xs={1} style={{ paddingLeft: '25px' }}>
              ...
              >
                ...
                <Popper
                  style={{ marginRight: '3%', marginTop: '0px' }}
                  open={OpenLogout}
          ...
          >
            ...
            <ListItem button key={text}>
              ...
              <Link to="/DashboardPage/StudylistNew" style={{ color: 'white' }}>
                <ListItem
                  button
                  onClick={() => {
                    setPath('/DashboardPage'), setOpen(true);
                  }}
                >
                  <ListItemIcon>
                    {' '}
                    <ReceiptOutlinedIcon style={{ color: 'white' }} />{' '}
                  </ListItemIcon>
              ...
              <Collapse in={dropdownOpen1} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  <Link
                    to="/DashboardPage/local"
                    onClick={() => {
                      setPath('/DashboardPage/local');
                    }}
                  >
                    <ListItem
                      button
                  ...
                  <Link
                    to="/DashboardPage/UploadFile"
                    onClick={() => {
                      setPath('/DashboardPage/UploadFile');
                    }}
                  >
                    <ListItem
                      button
              ...
              <Collapse in={dropdownOpen2} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  <Link
                    to="/DashboardPage/AWSAccessKey"
                    onClick={() => setPath('/DashboardPage/AWSAccessKey')}
                  >
                    <ListItem
                      button
                      className={classes.nested}
                  ...
                  <Link
                    to="/DashboardPage/GoogleAccessKey"
                    onClick={() => setPath('/DashboardPage/GoogleAccessKey')}
                  >
                    <ListItem
                      button
                      className={classes.nested}
              ...
              <Collapse in={dropdownOpen3} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  <Link
                    to="/DashboardPage/ReactComponent"
                    onClick={() => setPath('/ReactComponent')}
                  >
                    <ListItem
                      button
                      className={classes.nested}
                      onClick={handleClickDropdown3}
                    >
                      <ListItemIcon>
                  ...
                  <Link
                    to="/DashboardPage/AngularComponent"
                    onClick={() => setPath('/AngularComponent')}
                  >
                    <ListItem
                      button
                      className={classes.nested}
                  ...
                  <Link
                    to="/DashboardPage/HTMLComponent"
                    onClick={() => setPath('/HTMLComponent')}
                  >
                    <ListItem
                      button
                      className={classes.nested}
