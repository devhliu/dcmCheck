...
const SuccessPage = () => {
  ...
  const getStripeID = async (token1) => {
    ...
    const hello = await fetch(`https://dcm-cloud.herokuapp.com/api/v1/stripe/getId`, requestOptions)
    ...
    <>
      <section className='success_section'>
        <div className=" Successpage_card">
          ...
          <h1 style={{ color: '#5ABC71' }}><strong>Thanks for Subscription</strong></h1>
          <img src='assets/right.png' width={70} style={{ margin: '35px 0' }} />
          <p>We received your purchase request;<br /> we'll be in touch shortly!</p>
          <div className='row '>
            <button onClick={() => history.push('/studylist')} type='button' style={{ width: '100%', height: 46, marginTop: '5%' }} className='SuccessPagebtn btn-primary btn-block '>
              To Studylist
            </button>
          </div>
