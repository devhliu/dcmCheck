...
import DCMCloud from '@ohif/core';
    ...
    <>
      ...
      <div className="study-list-container">
        <StudyList
          ...
          // Rows
          studies={studies}
          onSelectItem={(studyInstanceUID, bucket) => {
            var keyaVal = '';
            if (bucket == 'google') {
              keyaVal =
                localStorage.getItem('GoogleBucketName') +
                '^' +
                ...
                '^' +
                localStorage.getItem('GoogleBucketType');
            } else {
              keyaVal =
            ...
            //const googlekeya = new Buffer.from(googlekeyaVal).toString('base64');
            const viewerPath = RoutesUtil.parseViewerPath(appConfig, server, {
              studyInstanceUIDs: studyInstanceUID + '^' + keya,
            });
...
async function _fetchStudies(
  ...
  queryFiltersArray.forEach(filter => {
    ...
    console.log(searchStudiesPromise, 'searchStudiesPromise');
  });
