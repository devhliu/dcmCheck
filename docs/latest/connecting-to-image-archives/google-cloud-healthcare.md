...
1. Open the `config/google.js` file and change `YOURCLIENTID` to your Client ID
   value.
1. Run the DCMCloud Viewer using the config/google.js configuration file
...
The DCMCloud Viewer Docker container can be connected to Google Cloud Healthcare
by providing a Client ID at runtime. This is a very simple method to get up and
running.
...
```bash
docker run --env CLIENT_ID=$CLIENT_ID --publish 5000:80 dcmcloud/viewer:latest
```
